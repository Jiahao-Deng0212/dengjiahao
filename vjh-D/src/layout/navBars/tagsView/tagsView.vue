...
// 定义变量内容
const tagsRefs = ref<RefType>([]); // 动态获取ref
...
const storesRoutesList = useMenuList();
const { themeConfig } = storeToRefs(storesThemeConfig);
const { tagsViewRoutesList } = storeToRefs(storesTagsViewRoutes);
const { menuList } = storeToRefs(storesRoutesList);
...
const addBrowserSetSession = (tagsViewList: Array<object>) => {
	Local.setTarget('sessionStorage').set('tagsViewList', tagsViewList);
};
// 获取 pinia 中的 tagsViewRoutesList 列表
const getTagsViewRoutes = async () => {
	state.routeActive = await setTagsViewHighlight(route);
	state.routePath = (await route.meta.isDynamic) ? route.meta.isDynamicPath : route.path;
	state.tagsViewList = [];
	state.tagsViewRoutesList = tagsViewRoutesList.value!;
	initTagsView();
};
// pinia 中获取路由信息：如果是设置了固定的（isAffix），进行初始化显示
const initTagsView = async () => {
	if (Local.setTarget('sessionStorage').get('tagsViewList') && getThemeConfig.value.isCacheTagsView) {
		state.tagsViewList = await Local.setTarget('sessionStorage').get('tagsViewList');
	} else {
		// 获取isAffix = true
...
const onTagsClick = (v: RouteItem, k: number) => {
	...
	if (getThemeConfig.value.layout === 'columns') {
		const item: RouteItem = menuList.value.find((r: RouteItem) => r.path.indexOf(`/${v.path.split('/')[1]}`) > -1);
...
// 页面加载时
onMounted(() => {
	// 初始化 pinia 中的 tagsViewRoutesList 列表
	getTagsViewRoutes();
	initSortable();
});
// 路由更新时（组件内生命钩子）
