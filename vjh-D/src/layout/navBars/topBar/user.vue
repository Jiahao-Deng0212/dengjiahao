<template>
	<div class="layout-navbars-breadcrumb-user pr15" :style="{ flex: layoutUserFlexNum }">
		<!-- 字体大小 -->
		<!-- <el-dropdown :show-timeout="70" :hide-timeout="50" trigger="hover" @command="onComponentSizeChange">
...
const onHandleCommandClick = (path: string) => {
	if (path === 'logOut') {
		...
		window.open('https://github.com/Jiahao-Deng0212/dengjiahao/tree/main/vjh-D');
	} else {
		router.push(path);
	}
};
...
const onComponentSizeChange = (size: string) => {
	Local.setTarget('localStorage').remove('themeConfig');
	themeConfig.value.globalComponentSize = size;
	Local.setTarget('localStorage').set('themeConfig', themeConfig.value);
...
const initI18nOrSize = (value: string, attr: string) => {
	(<any>state)[attr] = Local.setTarget('localStorage').get('themeConfig')[value];
};
// 页面加载时
onMounted(() => {
